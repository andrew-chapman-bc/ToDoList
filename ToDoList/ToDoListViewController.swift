...
class ToDoListViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "ShowDetail" {
            ...
            destination.toDoItem = toDoItems.itemsArray[indexPath.row]
        } else {
...
extension ToDoListViewController: UITableViewDelegate, UITableViewDataSource, ListTableViewCellDelegate {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        print("numberofrowsinsection: \(toDoItems.itemsArray.count)")
        return toDoItems.itemsArray.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        print("cellforrowat indexpath.row=\(indexPath.row), cell \(toDoItems.itemsArray[indexPath.row])")
